...
{chart_lines}
   ...
   Partition key: ingest_date (string, yyyy-MM-dd)

2. validation_results_{suffix} — rule validation outputs.
   Columns:
{validation_lines}
...
def nl_query(event, path_params, body, **kwargs):
    """
    Translate a natural-language question into Athena SQL (or flag it for
    deep narrative analysis) and return results.
    """
    org_id = path_params.get('orgId')
    ...
    system_prompt = _build_nl_system_prompt(org_id)

    try:
        claude_resp = invoke_claude_model(
            inference_profile_id=MODEL_ID,
            body={
                'anthropic_version': 'bedrock-2023-05-31',
                'max_tokens': 1024,
        ...
        print(f"nl_query: bedrock invocation failed: {e}")
        return response(502, {'error': 'LLM call failed', 'code': 'LLM_ERROR'})

    if not isinstance(claude_resp, dict) or 'mode' not in claude_resp:
        return response(502, {
            'error': 'LLM returned an unexpected response shape.',
            'code': 'LLM_BAD_RESPONSE',
        })

    mode = claude_resp.get('mode')

    if mode == 'needs_deep_analysis':
        scope_sql_raw = claude_resp.get('scope_sql', '')
        reason = claude_resp.get('reason', '')
        try:
        ...
        # Run a row-count probe so the UI can show estimated work.
        try:
            count_result = analytics_helpers.run_athena_query(
                f"SELECT count(*) FROM ({scope_sql})",
                org_id,
            )
            estimated_rows = int(count_result['rows'][0][0]) if count_result['rows'] else 0
        except analytics_helpers.AthenaQueryError as e:
            return response(400, {
                'error': f'Scope query failed: {e.message}',
                'code': 'SCOPE_QUERY_FAILED',
                'sql': scope_sql,
            })

        estimated_rows = min(estimated_rows, analytics_helpers.MAX_DEEP_SCOPE_LIMIT)
        return response(200, {
            'mode': 'needs_deep_analysis',
            'reason': reason,
            'scope_sql': scope_sql,
    ...
    if mode != 'sql':
        return response(502, {
            'error': f"LLM returned unknown mode '{mode}'.",
            'code': 'LLM_BAD_RESPONSE',
        })

    sql_raw = claude_resp.get('sql', '')
    viz_type = claude_resp.get('viz_type', 'table')
...
def nl_query_deep(event, path_params, body, **kwargs):
    """
    Opt-in deep mode: execute a validated scope SELECT and run a per-row
    Claude extraction against narrative columns, aggregating answers.
    """
    org_id = path_params.get('orgId')
    ...
    with ThreadPoolExecutor(max_workers=10) as pool:
        ...
        for fut in futures:
            ...
            try:
                ...
                print(f"deep extraction future failed at idx={idx}: {e}")
                extracted[idx] = 'unknown'

    out_columns = list(columns) + [{'name': 'extracted_value', 'type': 'string'}]
    out_rows = [r + [v] for r, v in zip(rows, extracted)]
