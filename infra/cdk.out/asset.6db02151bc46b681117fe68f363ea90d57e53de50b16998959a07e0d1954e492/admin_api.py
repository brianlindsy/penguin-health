...
def _fetch_run_documents(run_id, org_id, claims, allowed):
    ...
    response stays consistent with the single-run endpoint.
    """
...
def list_validation_runs(event, path_params, **kwargs):
    ...
    Optional query params:
      ...
      - limit: cap the number of runs returned (default 50 when include=details,
        unbounded otherwise; hard max 200 with details to keep responses under
        API Gateway's 6 MB ceiling).
    """
    org_id = path_params.get('orgId')
    ...
    if include_details:
        ...
        with ThreadPoolExecutor(max_workers=DETAILS_FETCH_WORKERS) as pool:
            doc_lists = list(pool.map(
                lambda r: _fetch_run_documents(r['validation_run_id'], org_id, claims, allowed),
                runs,
            ))
