...
class CirclesOfCareSplitter(BaseCsvSplitter):
    ...
    # Whether to filter visits by visittype (substring match, case-sensitive).
    INTAKE_ONLY = True
    INCLUDED_VISIT_TYPES = ("Intake Screening", "BedDay-Psych", "BedDay-Detox")
    ...
    def split(self, csv_content: str, filename: str) -> List[Tuple[str, str]]:
        ...
        for visit_id, rows in groups.items():
            if self.INTAKE_ONLY:
                ...
                is_included = any(
                    len(r) > visittype_idx
                    and any(vt in str(r[visittype_idx]) for vt in self.INCLUDED_VISIT_TYPES)
                    for r in rows
                )
                if not is_included:
                    continue
