...
EXTRACT_FROM_ROWS_TOOL = {
    ...
    "description": (
        ...
        "IMPORTANT: `rows` must be an array of OBJECTS like "
        '[{"clientvisit_id": "abc", "answer": "text..."}, ...], NOT the '
        "raw nested-array shape that run_sql returns. To convert a run_sql "
        "result, pair each entry of `result.rows` with `result.columns` by "
        "name. If run_sql returned "
        '{"columns": [{"name": "clientvisit_id"}, {"name": "answer"}], '
        '"rows": [["abc", "..."], ["def", "..."]]}, pass '
        '[{"clientvisit_id": "abc", "answer": "..."}, '
        '{"clientvisit_id": "def", "answer": "..."}]. '
        "Passing an empty array will fail with EMPTY_ROWS."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
            "rows": {
                "type": "array",
                "description": (
                    "Array of row OBJECTS keyed by column name, e.g. "
                    '[{"clientvisit_id": "...", "answer": "..."}]. Must '
                    "include the narrative columns the extractor should read."
                ),
                "items": {"type": "object"},
            },
            "question": {
...
def make_extract_from_rows_handler(
    ...
    def handler(tu_input: dict) -> dict:
        ...
        if len(rows) == 0:
            return {
                "error": (
                    "rows is empty. You must pass an array of row objects, "
                    "where each object maps column names to values. To turn a "
                    "run_sql result into rows, pair each row in result.rows "
                    "with result.columns by name. Example: if run_sql returned "
                    '{"columns": [{"name": "clientvisit_id"}, {"name": "answer"}], '
                    '"rows": [["abc", "narrative text..."]]}, pass '
                    '[{"clientvisit_id": "abc", "answer": "narrative text..."}].'
                ),
                "code": "EMPTY_ROWS",
            }
