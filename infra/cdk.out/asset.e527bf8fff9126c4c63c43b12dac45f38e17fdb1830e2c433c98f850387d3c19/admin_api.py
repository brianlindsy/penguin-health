...
- run_sql(sql): execute a SELECT (auto-LIMIT applied). Returns
  {{columns: [{{name, type}}], rows: [[v1, v2, ...], ...], row_count}}.
  Note that `rows` is a list of POSITIONAL arrays, parallel to `columns`.
- extract_from_rows(rows, question): per-row Claude extraction of a
  short string from narrative fields. Cap 200 rows per call.
  `rows` here is DIFFERENT from run_sql's: it must be an array of
  OBJECTS keyed by column name, like
  [{{"clientvisit_id": "abc", "answer": "text..."}}, ...].
- aggregate(rows, group_by, agg, sum_field?, order_by?): in-memory
  GROUP BY. `rows` is also an array of objects. To aggregate
  extract_from_rows output, build objects like
  [{{"extracted_value": "Police"}}, {{"extracted_value": "Hospital"}}, ...].
- finalize(columns, rows, viz_type?, explanation?): emit final answer
  and end the loop. `rows` here is POSITIONAL (parallel to columns), same
  shape as run_sql returns. You MUST call finalize exactly once when done.

Canonical pattern for "count <thing> extracted from narrative":
  1. run_sql to fetch one row per entity with the narrative columns.
  2. Convert the run_sql result to row-OBJECTS keyed by column name.
  3. extract_from_rows(rows=<objects>, question=<what to pull out>).
  4. Build row-OBJECTS like [{{"extracted_value": v}} for v in extracted].
  5. aggregate(rows=<objects>, group_by=["extracted_value"], agg="count").
  6. finalize with the aggregate result, mapping aggregate's
     {{columns, rows}} straight into finalize's same fields.
...
- finalize.rows should be the FINAL shape shown to the user. Do not
  include intermediate columns the user didn't ask for (e.g. omit the
  raw `answer` narrative column from the final result unless the user
  explicitly asked for it).
- If a tool returns an error, READ THE ERROR MESSAGE CAREFULLY before
  retrying. Do not repeat the same call with the same shape — the error
  tells you what was wrong.
- Once you have the data you need, call finalize IMMEDIATELY. Do not
  do extra exploration after the answer is in hand.
...
def _agent_worker_run(org_id: str, job_id: str, item: dict) -> dict:
    ...
    def on_step(step_info):
        # Persist a lightweight progress marker so the polling UI can show
        # "Step 3: thinking" while a Bedrock turn is in flight.
        try:
            deep_jobs_table.update_item(
                Key={'pk': _deep_job_pk(org_id), 'sk': _deep_job_sk(job_id)},
                UpdateExpression=(
                    'SET step_count = :n, current_step_label = :l, updated_at = :u'
                ),
                ExpressionAttributeValues={
                    ':n': step_info['step'],
