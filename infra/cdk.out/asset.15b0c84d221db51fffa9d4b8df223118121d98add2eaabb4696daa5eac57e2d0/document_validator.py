...
def validate_all_rules(rules, chart_text, extracted_fields_by_rule):
    ...
    system_prompt = """You are a Healthcare Compliance Auditor. Validate each rule against the chart text.
...
[
  {"rule_id": "1", "status": "PASS", "reasoning": "Brief explanation"},
  {"rule_id": "2", "status": "FAIL", "reasoning": "Brief explanation of failure"},
  ...
]
...
def validate_document(data, filename, config, org_id, validation_run_id):
    ...
    if not enabled_rules:
        ...
        rule_results = []
        results_by_id = {r.get('rule_id'): r for r in validation_results}
