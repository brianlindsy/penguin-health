...
def make_s3_spill(s3_client, bucket: str, job_id: str) -> Callable[[bytes, str], str]:
    ...
    Keys live under a per-job prefix so a single lifecycle rule on the
    bucket can expire them along with the DynamoDB TTL.
    """
    def spill(data: bytes, suffix_hint: str) -> str:
        key = f"agent-jobs/{job_id}/{int(time.time() * 1000)}-{uuid.uuid4().hex[:8]}-{suffix_hint}"
...
def agent_io_bucket() -> str:
    """Bucket used for spilling large intermediate payloads. Falls back to
    a sentinel so local tests / lambda cold paths fail loudly rather than
    silently writing to a stray bucket."""
    return os.environ.get("AGENT_IO_BUCKET", "")
