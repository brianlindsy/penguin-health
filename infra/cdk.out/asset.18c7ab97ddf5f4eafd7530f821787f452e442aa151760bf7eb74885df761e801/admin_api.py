...
AGENT_IO_BUCKET = os.environ.get('AGENT_IO_BUCKET', '')
...
- When a question asks to count or group by a value buried in narrative,
  the canonical pattern is: run_sql to fetch rows + narrative columns →
  extract_from_rows → aggregate → finalize.
...
def _agent_io_bucket() -> str:
    return AGENT_IO_BUCKET
...
def _agent_worker_run(org_id: str, job_id: str, item: dict) -> dict:
    ...
    try:
        ...
        print(f"agent_worker: job {job_id} failed: {e.code}: {e.message}")
        deep_jobs_table.update_item(
            Key={'pk': _deep_job_pk(org_id), 'sk': _deep_job_sk(job_id)},
            UpdateExpression=(
        ...
        print(f"agent_worker: job {job_id} unexpected failure: {e}")
        deep_jobs_table.update_item(
            Key={'pk': _deep_job_pk(org_id), 'sk': _deep_job_sk(job_id)},
            UpdateExpression='SET #s = :s, #err = :err, updated_at = :u',
            ExpressionAttributeNames={'#s': 'status', '#err': 'error'},
