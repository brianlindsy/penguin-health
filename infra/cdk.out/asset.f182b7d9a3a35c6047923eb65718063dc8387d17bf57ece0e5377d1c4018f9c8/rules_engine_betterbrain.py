...
def evaluate_llm_rule_structured(rule_config, fields, data=None):
    ...
    print(f"DEBUG LLM: Rule config: {rule_config}")
    ...
    if rule_config['id'] == "1":
        fields_to_extract = [
            ...
            },
            {
                ...
                'type': 'string',
                'description': 'The actual method of contact used to communicate with the recipient. Look at the Billing Matrix and Location fields.'
            }
        ]
        rule_description = f"Check to make sure the Recipient field is consistent with the actual documented method of contact, specified by the Billing Matrix and Location fields."
        notes = [
            "'Face to Face' Recipient covers In-Person AND Video (Visual) Telehealth. 'Telephone' Recipient is for Audio-Only."
        ]
        general_system_prompt = f"""You are a Healthcare Compliance Auditor. You will be given a Rule to validate, the patient Chart Text, and the list of fields you should first extract from the Chart Text to help another LLM validate the rule. Your only purpose is to extract the fields, and return them in a JSON object."""
        general_system_prompt += f"\nPlease respond with JSON, with the key: 'fields'. The fields should be a list of objects, each with the keys: 'name', 'type' (string or number), and 'description'."

        fields_list_to_extract = [f['name'] for f in fields_to_extract]
        properties = {
            f['name']: {
                'type': f['type'],
        ...
        fields_json_schema = {
            ...
            "description": "List of fields to extract from the chart data.",
            "type": "object",
            "properties": {
                "fields": {
                    "type": "array",
                    "items": {
                        ...
                        "required": fields_list_to_extract
                    }
                }
            },
            "required": ["fields"]
        ...
        if response_json is None:
            return 'ERROR', 'No JSON found in Claude response (field extraction)', 'No JSON found in Claude response (field extraction)'

        fields = response_json['fields']
        print(f"DEBUG LLM: Fields extracted: {fields}")

        # now make the next LLM request to validate the rule
        validation_system_prompt = f"""You are a Healthcare Compliance Auditor. You will be given a Rule to validate, the patient Chart Text, and a list of fields / information already extracted from the Chart Text to help validate the rule. Your only purpose is to validate the rule, and return a JSON object with the keys: 'status' and 'reasoning'. The status should be one of: 'PASS', 'FAIL', 'SKIP'. The reasoning should be a short explanation of the reason for the status."""
        ...
        validation_response_json = invoke_claude_model(
            inference_profile_id=model_id,
            body=validation_json_body,
            return_json_only=True,
            raise_on_error=True,
            retries=1
        )
        ...
        if validation_response_json is None:
            return 'ERROR', 'No JSON found in Claude response (rule validation)', 'No JSON found in Claude response (rule validation)'

        status = validation_response_json['status']
        reasoning = validation_response_json['reasoning']
