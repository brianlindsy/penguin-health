...
- run_sql(sql): execute a SELECT (auto-LIMIT applied). Returns
  ...
  `from_run_id` to extract_from_rows / aggregate — do NOT copy rows
  into their inputs.
- extract_from_rows(from_run_id, question): per-row Claude extraction
  ...
  Returns {{run_id, row_count, preview_results}}.
...
- ALWAYS use `from_run_id` to pass rows between tools. Do not try to
  ...
  run_id and the server will load the rows. Copying is brittle and the
  tools will reject empty `rows` arrays.
