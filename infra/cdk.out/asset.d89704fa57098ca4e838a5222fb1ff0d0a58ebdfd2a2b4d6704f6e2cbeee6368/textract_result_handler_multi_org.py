...
def process_and_store_results(job_id, source_file_key, config, textract_response=None):
    ...
    try:
        ...
        # Where the processed JSON lands so validation can find it.
        # Charts: data/{ingest_date}/{ingest_ts}__... (consumed by the rules
        # engine via date-folder listings).
        # IRP: textract-processed/irp/ (separate pipeline, not validation).
        from datetime import timezone as _tz
        _now_utc = datetime.now(_tz.utc)
        ingest_date = _now_utc.strftime('%Y-%m-%d')
        ingest_ts = _now_utc.strftime('%Y%m%dT%H%M%SZ')
        if is_irp:
            processed_prefix = config.get('TEXTRACT_IRP_PROCESSED', 'textract-processed/irp/')
        else:
            processed_prefix = f"data/{ingest_date}/{ingest_ts}__"
        ...
        if len(encounters) > 1:
            ...
            for idx, encounter in enumerate(encounters):
                ...
                output_key = f"{processed_prefix}{filename}-{safe_encounter_id}-{timestamp}.json"
            ...
            output_key = f"{processed_prefix}{filename}-{timestamp}.json"
            s3_client.put_object(
                Bucket=config['BUCKET_NAME'],
                Key=output_key,
        ...
        manifest_key = f"{processed_prefix}{filename}-{timestamp}-batch-complete.json"
        s3_client.put_object(
            Bucket=config['BUCKET_NAME'],
            Key=manifest_key,
