"""HTTP handlers for the CentralReach integration's admin surface.
...
Scope is intentionally minimal. CentralReach is an ingestion path, not
a UI surface — extracted notes feed the rules engine via S3 and are
reviewed through the existing validation-results UI. The on-demand
`run` endpoint exists for engineers and operators to kick a manual
pass; the listing endpoints exist so an operator can see whether
scheduled passes succeeded without dropping into the Step Functions
console.

Authorization:
  All four routes require the caller to have `view` or `run` on the
  `Compliance Audit` category for the org — CentralReach is a
  compliance-audit data source, so gating it on that category keeps
  the permission model consistent without inventing a new category
  for one integration.

The state-machine ARN is read from `CENTRALREACH_STATE_MACHINE_ARN`
(set by the CDK component in `infra/components/centralreach.py`).
Per-org dispatch happens inside the state machine, not in this Lambda
— we pass `org_id` as the SFN input.
"""
...
_STATE_MACHINE_ARN_ENV = "CENTRALREACH_STATE_MACHINE_ARN"
_CENTRALREACH_PERMISSION_CATEGORY = "Compliance Audit"
...
def _ensure_can_view(claims, org_id):
    return perms_module.can_view_category(
        claims, org_id, _CENTRALREACH_PERMISSION_CATEGORY,
    )


def _ensure_can_run(claims, org_id):
    return perms_module.can_run_category(
        claims, org_id, _CENTRALREACH_PERMISSION_CATEGORY,
    )
...
def _state_machine_arn():
    ...
    if not arn:
        raise RuntimeError(
            ...
            "the CentralReach CDK component must wire it into this Lambda."
        )
    return arn


# Fields that should never be surfaced to the API. None of these live
# in the CENTRALREACH_CONFIG item today, but we filter defensively in
# case a future rev. of the schema adds something the schema-validator
# misses.
...
def _scrub_config(item):
    """Deep-redact obviously-sensitive keys before returning
    CENTRALREACH_CONFIG.
...
@audited(action="read", resource_type="CentralReachConfig",
         purpose_of_use="OPERATIONS", call_type="centralreach_config_get")
def get_config(event, path_params, authorize_fn, **_):
    ...
    if not _ensure_can_view(claims, org_id):
        return _response(403, {"error":
                               f"{_CENTRALREACH_PERMISSION_CATEGORY}:view permission required"})
    try:
        item = cr_config.load_centralreach_config(org_id)
...
@audited(action="execute", resource_type="CentralReachIngestRun",
         purpose_of_use="OPERATIONS", call_type="centralreach_run_trigger")
def trigger_run(event, path_params, body, authorize_fn, **_):
    ...
    if not _ensure_can_run(claims, org_id):
        return _response(403, {"error":
                               f"{_CENTRALREACH_PERMISSION_CATEGORY}:run permission required"})
    ...
    try:
        cr_config.load_centralreach_config(org_id)
    except CentralReachOrgNotConfigured as e:
...
@audited(action="read", resource_type="CentralReachIngestRun",
         purpose_of_use="OPERATIONS", call_type="centralreach_runs_list")
...
@audited(action="read", resource_type="CentralReachIngestRun",
         purpose_of_use="OPERATIONS", call_type="centralreach_run_get")
def get_run(event, path_params, authorize_fn, **_):
    ...
    return _response(200, {
        ...
        # (processed_count, skipped_count, failure_count, vendor)
        # without a console roundtrip.
