...
_CHART_COLUMN_NOTES = {
    ...
    ("catholic-charities-multi-org", "visit_type_name_21b"): (
        "Free-text visit type. Observed values include 'No-Show / Cancel'. "
        "Use SELECT DISTINCT visit_type_name_21b to discover the full set."
    ),
}
...
_CHART_COLUMNS = {
    "circles-of-care": [
        ...
        "problem_list_order", "diagnosis_on_service", "payer_description",
        "fake_client_id2", "clientvisit_id2", "first_referral",
        "question_text", "answer",
        "type", "episode_id2", "cptcode", "first_name", "last_name", "rate",
        "initial_appt", "agegroup", "diagnose_on_visit",
    ],
    ...
    "catholic-charities-multi-org": [
        ...
        "tx_plus_intervention_documentation_47g", "plan_documentation_47e",
...
# ---- Schema metadata (Bedrock-audited) --------------------------------
#
# Per-org metadata files emit role/values/do_not_use_for per column, plus
# top-level column_groups, join_keys, and row_grain. Generated by feeding
# a CSV sample through Bedrock Claude (see scripts/audit_schema.py for the
# prompt). Column names in the JSON are in CSV header form ("21b_Visit_Type_Name");
# `_csv_to_athena_name` normalizes them to the Athena form used in
# _CHART_COLUMNS ("visit_type_name_21b").

_METADATA_DIR = pathlib.Path(__file__).parent

_METADATA_FILES = {
    "catholic-charities-multi-org": "schema_metadata_catholic_charities.json",
}


# CSV column headers look like "21b_Visit_Type_Name" or "30_31_Form_Q_and_A".
# Glue/Athena moves the leading numeric token to the end and lowercases.
_CSV_LEAD_RE = re.compile(r"^(\d+[a-z]?(?:_\d+)?)_(.+)$", re.IGNORECASE)


def _csv_to_athena_name(csv_name: str) -> str:
    """Map a CSV header (e.g. '21b_Visit_Type_Name') to its Athena column
    name (e.g. 'visit_type_name_21b'). Idempotent for names that don't
    match the leading-numeric pattern."""
    m = _CSV_LEAD_RE.match(csv_name)
    if not m:
        return csv_name.lower()
    prefix, rest = m.group(1), m.group(2)
    return f"{rest.lower()}_{prefix.lower()}"


def _load_org_metadata(org_id: str) -> Optional[dict]:
    """Load and normalize per-org metadata. Returns None if no file exists."""
    fname = _METADATA_FILES.get(org_id)
    if not fname:
        return None
    path = _METADATA_DIR / fname
    if not path.exists():
        return None
    data = json.loads(path.read_text())
    # Re-key columns by Athena name so lookups in _build_org_schemas are O(1).
    by_athena = {}
    for entry in data.get("columns", []):
        csv_name = entry.get("name")
        if not csv_name:
            continue
        athena_name = _csv_to_athena_name(csv_name)
        by_athena[athena_name] = entry
    # Same for column_groups members.
    groups = []
    for g in data.get("column_groups", []):
        members = [_csv_to_athena_name(m) for m in g.get("members", [])]
        primary = g.get("primary")
        groups.append({
            "name": g.get("name"),
            "primary": _csv_to_athena_name(primary) if primary else None,
            "members": members,
            "note": g.get("note"),
        })
    # And join_keys.
    join_keys = {
        scope: [_csv_to_athena_name(c) for c in cols]
        for scope, cols in (data.get("join_keys") or {}).items()
    }
    return {
        "columns_by_athena": by_athena,
        "column_groups": groups,
        "join_keys": join_keys,
        "row_grain": data.get("row_grain"),
        "row_grain_explanation": data.get("row_grain_explanation"),
    }


def _filter_groups_to_known_cols(groups: list, known: set) -> list:
    """Drop group members the registry doesn't know about, and drop groups
    that end up empty. Audits can include columns that exist in the CSV
    but didn't land in the Athena table — silently dropping them here is
    better than surfacing references the agent can't actually query."""
    out = []
    for g in groups:
        ...
        if not members:
            continue
        primary = g.get("primary")
        if primary and primary not in known:
            primary = members[0]
        out.append({**g, "members": members, "primary": primary})
    return out
...
def _build_org_schemas():
    ...
    for org_id, cols in _CHART_COLUMNS.items():
        ...
        meta = _load_org_metadata(org_id) or {}
        meta_cols = meta.get("columns_by_athena", {})
        chart_columns = []
        for c in cols:
            entry = {
                "name": c,
            ...
            if note:
                entry["notes"] = note
            # Merge Bedrock-audited metadata when present. The audit's
            # `note` overrides the hand-written `notes` because the audit
            # was run against live data and is the more authoritative
            # source; hand-written notes are kept only as a fallback for
            # columns the audit didn't cover.
            m = meta_cols.get(c)
            if m:
                if m.get("role"):
                    entry["role"] = m["role"]
                ...
                if m.get("do_not_use_for"):
                    entry["do_not_use_for"] = m["do_not_use_for"]
        ...
        known_chart_cols = {c["name"] for c in chart_columns}
        schemas[org_id] = {
            ...
            "column_groups": _filter_groups_to_known_cols(
                meta.get("column_groups", []) or [], known_chart_cols,
            ),
            "join_keys": {
                scope: [c for c in cols if c in known_chart_cols]
                for scope, cols in (meta.get("join_keys") or {}).items()
            },
            "row_grain": meta.get("row_grain"),
            "row_grain_explanation": meta.get("row_grain_explanation"),
        }
    return schemas
...
def _first_non_ws_child(tok):
    """Return the first non-whitespace child of a grouped token, or None."""
    for c in getattr(tok, "tokens", []) or []:
        if not c.is_whitespace:
            return c
    return None


def _cte_names_from_with(parsed) -> set:
    """Collect the names introduced by a top-level `WITH ...` clause.

    These are NOT real tables (they don't need to be in the allowlist), but
    later `FROM <cte_name>` references will look like tables to the
    extractor. We pre-collect them so the allowlist check can exempt them.
    The CTE bodies (parenthesized SELECTs) still get walked for real
    tables, so a CTE referencing a disallowed base table is still caught.
    """
    names: set = set()
    saw_with = False
    for tok in parsed.tokens:
        if tok.is_whitespace:
            continue
        # CTE keyword token: ttype is Token.Keyword.CTE for WITH.
        if tok.ttype is not None and "CTE" in str(tok.ttype) and tok.normalized.upper() == "WITH":
            saw_with = True
            continue
        if saw_with:
            # Single CTE: Identifier whose first Name child is the CTE name.
            # Multiple CTEs: IdentifierList of those Identifiers.
            if isinstance(tok, IdentifierList):
                for sub in tok.get_identifiers():
                    n = sub.get_real_name()
                    if n:
                        names.add(_normalize_table_name(n))
            elif isinstance(tok, Identifier):
                n = tok.get_real_name()
                if n:
                    names.add(_normalize_table_name(n))
            # WITH consumes exactly one Identifier or IdentifierList.
            break
    return names
...
def _extract_table_names(parsed) -> list:
    """
    Walk the parsed statement and collect every base-table identifier that
    follows FROM or JOIN. Subquery aliases (e.g. `FROM (...) p`) emit the
    real tables from the inner SELECT, not the alias. CTE-defined names are
    NOT emitted here — they're tracked separately via _cte_names_from_with
    and exempted at the allowlist check.
    """
    tables = []
    from_seen = False

    def emit_from_token(tok):
        # A subquery in FROM position: Identifier whose first child is a
        # Parenthesis. The alias (if any) is NOT a table; the inner SELECT's
        # tables will be picked up by the recursive walk below.
        if isinstance(tok, Identifier):
            first = _first_non_ws_child(tok)
            if isinstance(first, Parenthesis):
                return
            tables.append(_normalize_table_name(tok.get_real_name() or str(tok)))
            return
        if isinstance(tok, Parenthesis):
            # Bare parenthesized subquery without alias — also not a table.
            return
        if isinstance(tok, IdentifierList):
            for sub in tok.get_identifiers():
                if isinstance(sub, Identifier):
                    first = _first_non_ws_child(sub)
                    if isinstance(first, Parenthesis):
                        continue
                    tables.append(_normalize_table_name(sub.get_real_name() or str(sub)))
                else:
                    tables.append(_normalize_table_name(str(sub)))
            return
        text = str(tok).strip()
        if text:
    ...
    def walk(tokens):
        ...
        for tok in tokens:
            ...
            # so tables inside CTE bodies and FROM-subqueries are still checked.
...
def validate_athena_sql(
    ...
    # Table allowlist. CTE-defined names are synthetic — they don't need
    # to be in the allowlist, but tables referenced from inside the CTE
    # body are still walked and checked.
    allowed = allowed_tables(org_id)
    cte_names = _cte_names_from_with(parsed)
