...
def deep_worker_handler(event, context):
    """
    Worker Lambda entry point. Loads the job, fans out per-row Claude
    extraction with bounded concurrency, writes progress to DynamoDB
    every PROGRESS_BATCH rows, and on completion stamps final viz_type
