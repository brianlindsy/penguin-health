...
class CatholicCharitiesSplitter(BaseCsvSplitter):
    ...
    def split(self, csv_content: str, filename: str) -> List[Tuple[str, str]]:
        ...
        - 21b_Visit_Type_Name not in EXCLUDED_VISIT_TYPE_NAMES
        ...
        for row in reader:
            ...
            # Filter: Exclude specific visit type names
            visit_type_name = row.get('21b_Visit_Type_Name', '').strip()
            if visit_type_name in self.EXCLUDED_VISIT_TYPE_NAMES:
                continue
