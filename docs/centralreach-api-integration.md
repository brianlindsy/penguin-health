...
lambda/multi-org/centralreach/
  ...
  parameters.py            # 14-day Eastern rolling window + env-var overrides (moved from rpa)
...
- `parameters.py` — 14-day Eastern rolling-window resolver + env-var overrides
...
makes the 14-day daily lookback affordable — after day 1,
...
| `extracted_fields.note_supervisor_name` | New. Bedrock-extracted supervisor name from the note's header/attribution area, NOT from the signature line. Extractor tries three sources in order: (a) a dedicated supervisor section (e.g. "Supervisor: Dr. Jane Doe"); (b) the provider section when the name inside carries a role label containing "supervis" case-insensitive (e.g. "Provider: Jane Doe, Supervising Analyst, BCBA-D" → "Jane Doe"); (c) a Participants-style checkbox block with a top-level Supervisor checkbox that's checked (credential sub-options BCBA/BCaBA/QBA — the checkbox is the signal, not the name text). If multiple credential sub-boxes are checked (ambiguous), falls back to the first entry of `note_supervisor_signature_names`. Compared against `note_supervisor_signature_names` by cross-check rules. |
| `extracted_fields.note_supervisor_signature_names` | New. Bedrock-extracted JSON list containing the name of every signer whose signature block carries a supervising-role label (case-insensitive contains "supervis" — matches "Supervisor", "Supervising Analyst", "Supervised by"). A note can legitimately carry more than one supervisor signature (co-supervisors, multi-approver forms); Rule 7's supervisor-signature check passes when the expected supervisor matches ANY entry in the list. Names appear in document order (earlier signature blocks first). Populated even when a block sits under a "Provider Signature" line — the role text inside the block is the signal, not the line label above it. Captures the common ABA case where the supervising BCBA is the sole signer on a solo note; in that case `note_provider_signature_name` and `note_supervisor_signature_names` carry the same signer's name (as a one-element list). Distinct from `note_supervisor_name` (attribution section, top of note). Rule 7 treats absence (omitted key — empty list) as "no signature." |
...
`supervisor_signature`) don't use a prefix. Nothing else sources those
values, so a `preview_` prefix would add noise without resolving
ambiguity.
...
2. **`note_fields_extractor.extract_note_fields`** — six structured
   ...
   `note_supervisor_signature_names` (JSON list of every name that
   signed as a supervisor — a note can carry more than one). Every
   field is optional — a `null` (or `[]` for the signature list) from
   Bedrock means "not present on the note," and the builder omits the
   corresponding key so a rule can distinguish absence from a false
...
(14-day Eastern rolling window ending yesterday). Non-Eastern orgs will
