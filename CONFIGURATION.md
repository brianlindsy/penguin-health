...
```bash
# 1. Update test event with your values
vim test-events/process-raw-charts-event.json
...
### Different orgs need different configs

**Problem:** Multiple organizations with different S3 buckets, tables, etc.

**Solution:** Use event payload config:
```bash
# Org 1
aws lambda invoke \
  --function-name rules-engine \
  --payload '{"config":{"ORGANIZATION_ID":"org1","BUCKET_NAME":"org1-bucket"}}' \
  response.json

# Org 2
aws lambda invoke \
  --function-name rules-engine \
  --payload '{"config":{"ORGANIZATION_ID":"org2","BUCKET_NAME":"org2-bucket"}}' \
  response.json
```